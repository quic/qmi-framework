...
static void parse_command(int argc, char *const argv[])
{
	...
	while ((command = getopt_long(argc, argv, "has:i:n:d:t:", longopts, NULL)) != -1) {
		switch (command) {
		case 'h':
			usage(argv[0]);
			exit(0);
...
)
{
	...
	case QMI_TEST_RESP_V01:
		printf("TEST: Async Test pong_valid: %d\n",((test_ping_resp_msg_v01 *)buf)->pong_valid);
...
{
	int i, rc, cb_data = 2;
	test_ping_req_msg_v01 req = {0};
	test_ping_resp_msg_v01 resp = {0};
	...
	for (i=0; i<num_msgs; ++i) {
		if (async_mode) {
			...
			if (rc != 0) {
				...
				printf("TEST: Async Test pong_valid: %d\n",resp.pong_valid);
				printf("TEST: Sync Test Response: %s\n",resp.pong);
			}
		}
...
{
	int i, rc, cb_data = 2;
	test_data_req_msg_v01 *data_req = {0};
	test_data_resp_msg_v01 *data_resp = {0};
	...
	printf("TEST: Data Test with %d data messages of size %d.\n",num_msgs,
	       msg_size);
	for (i=0; i<num_msgs; ++i) {
		if (async_mode) {
...
{
	int i, rc, cb_data = 2;
	test_data_ind_reg_req_msg_v01 data_ind_reg_req = {0};
	test_data_ind_reg_resp_msg_v01 data_ind_reg_resp = {0};
